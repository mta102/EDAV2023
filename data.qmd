...
The CDC estimated the weighted proportion of the sampled individuals giving a particular response in the survey, where the weighting attempts to correct any imbalances or biases in the sample that might arise due to the survey design or data collection methods.
...
The chosen dataset provides a number of interesting factors to be examined in the context of vaccine hesitance.

The research questions can be divided into 3 parts:
...
Below, we have outlined the fields we plan to leverage in order to explore potential relationships between our dependent variable - vaccine hesitance response rates - and various independent variables such as geography, age, and ethnicity.

Time Window:
The beginning of the dataset where vaccine hesitance was stable: [Karl to add]
```{r}
...
Geography = National
Group Name = Each segment, as defined below:
...
In addition to looking at participant demographics as independent variables, we would also like to study changes in hesitance at particular points in time and changes over a large period of time (2 years, from Apr/May 2021 to Jun 2023). As such, we will use the Time Period field as another variable for this analysis. While the data is broken down between weekly and monthly samples, we will be focusing on monthly data for our study. This is because we do not expect significant changes in hesitance on a weekly basis, and attempting to compare weekly data could be subject to sampling error due to the random-dialed study design.

Part 3: Propose high-level recommendations for segments of the population that need further public health interventions 
#---#

We will draw this conclusion based on which segments of the populated showed the least change in covid vaccine hesitancy between 2021 and 2023.
...
-	Writing R-code to auto rank every possible segment’s hesitance by statistical significance at a given point in time (i.e. using p-values and test-statistics from Chi-sq tests).
...
Part 2: Analyse which segments showed the greatest reduction in covid vaccine hesitancy between 2021 and 2023
#---#
Our next step will be to analyse which segments showed the greatest reduction in COVID vaccine hesitancy between 2021 and 2023. To perform this step, we will leverage several visuals that can provide insights into trends of hesitancy over time:
...
Part 3: Propose high-level recommendations for segments of the population that need further public health interventions 
Based on our findings, our last step in our analysis approach will be to propose high-level recommendations for segments of the population that need further public health interventions.
...
# covid <- fread("National_Immunization_Survey_Adult_COVID_Module__NIS-ACM___COVIDVaxViews__Data___Centers_for_Disease_Control_and_Prevention__cdc.gov_.csv")

covid <- fread("c:/temp/covid.csv")
...
na_estimates <- covid |>
  ...
  mutate(missing_count_ind_name = sum(missing_count)) |>
  ungroup()

na_estimates = na_estimates |> filter(!grepl(":",Geography) & !grepl("-",Geography) & `Time Type` == "Monthly" ) #remove regions & sub-states & weekly
...
na_estimates |> ggplot(aes(y = `Time Period`,x = Year,fill = sum(missing_count))) +
  geom_tile() +
  labs(title = "Missing Values by Group Name",
       x = "Geography",
       y = "Count of NA") +
...
na_estimates |> filter(`Group Name` == "Age") |>  
  ggplot(aes(x = `Group Category`)) +
...
na_estimates |>  ggplot(aes(x = fct_infreq(`Group Name`))) +
  geom_bar(aes(y = (..count..)/sum(..count..))) +
  labs(title = "Missing Values by Group Name",
       x = "Group Name",
       y = "Count of NA") +
...
na_estimates |>  ggplot(aes(x = fct_infreq(Geography))) +
  geom_bar(aes(y = (..count..)/sum(..count..))) +
  labs(title = "Missing Values by Geography",
       x = "Geography",
       y = "Count of NA") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  scale_y_continuous(labels = scales::percent)
...
NA_is_zero_proof2 = covid |> group_by(`Estimate (%)`) |>  summarize(count = n())  |> filter(`Estimate (%)`<=0.2 | is.na(`Estimate (%)`))

na_estimates |>  ggplot(aes(x = `95% CI (%)`)) +
  geom_bar(aes(y = (..count..)/sum(..count..))) +
...
ggplot(na_estimates,
       aes(x = missing_count, y = reorder(`Indicator Name`,
                                          missing_count_ind_name))) +
  geom_bar(stat = "identity", fill = "cornflowerblue") +
  labs(title = "Missing Values by Indicator Names",
       x = "Count of NAs",
       y = "Indicator Names") +
  theme_bw()
...
na_estimates_no_sample_outlier <- na_estimates |>
  mutate(lower_outlier = quantile(`Sample Size`, 0.25) - 1.5 *
           IQR(`Sample Size`),
         upper_outlier = quantile(`Sample Size`, 0.75) + 1.5 *
           IQR(`Sample Size`)) |>
  filter(`Sample Size` >= lower_outlier & `Sample Size` <= upper_outlier)

# Plot histogram of Sample Sizes
ggplot(na_estimates_no_sample_outlier, aes(`Sample Size`)) +
  geom_histogram(boundary = 0,
                 color = "blue", fill = "white") +
  labs(title = "Sample Sizes with Missing Vaccine Response Rates",
       x = "Sample Sizes",
       y = "Count of Samples") +
  theme_bw()

# Create boxplots
ggplot(na_estimates, aes(x = `Sample Size`, y = `Time Type`)) + 
    geom_boxplot()

```
The histogram above shows that missing values are more prevalent among segments with smaller sample sizes?

```{r}

na_estimates_analysis <- na_estimates |>
  filter(`Time Type` == "Monthly" & `Indicator Name` == "Vaccination uptake and intention") |>
  filter(Geography == "National") |>
  group_by(`Group Name`) |>
  mutate(missing_count_group_name = sum(missing_count)) |>
  ungroup()


ggplot(na_estimates_analysis,
       aes(x = missing_count,
           y = reorder(`Group Name`, missing_count_group_name))) +
  geom_bar(stat = "identity", fill = "cornflowerblue") +
  labs(title = "Missing Values by Group Name",
       x = "Count of NAs",
       y = "Group Names") +
...
ggplot(na_estimates_analysis,
       aes(x = `Indicator Category`, y = `Group Name`)) +
  geom_tile(aes(fill = `missing_count`), color = "white") +
  coord_fixed() +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
  labs(x = "Day", y = "Fire Indices", fill = "Fire Danger Level") +
  ggtitle("Fire Danger Levels By Station & Day")


covid_vacc_nation <- covid |>
  filter(`Indicator Name` == "Vaccination uptake and intention") |>
  filter(Geography == "National")

# Pivot wider to pivot indicators to columns
covid_vacc_nation_wide <- covid_vacc_nation |>
  pivot_wider(id_cols = c(`Group Name`, `Group Category`, `Time Period`,
                          Year, `Time Type`),
              names_from = "Indicator Category",
              values_from = "Estimate (%)")

#miss_df <- missing_data.frame(covid_vacc_nation_wide)
# 
# image(miss_df)
