...
covid_2021_analysis_allstates <- covid %>%
  ...
  mutate(first_two_char = str_sub(Geography, 1, 2),
         state_clean = ifelse(first_two_char == "PA", "Pennsylvania", ifelse(
...
covid_2021_analysis_allstates_map <- covid_2021_analysis_allstates %>%
  select(state_clean, `Estimate (%)`) |>
  #rownames_to_column("state_clean") %>% 
  transmute(region = tolower(state_clean), value = `Estimate (%)`)
...
state_group_analysis <- covid |>
  ...
  mutate(first_two_char = str_sub(Geography, 1, 2),
         state_clean = ifelse(first_two_char == "PA", "Pennsylvania", ifelse(
            ...
            first_two_char == "IL", "Illinois", Geography)))),
         n_hesitant = round(`Estimate (%)`/100 * `Sample Size`, 0)) |>
  group_by(state_clean, `Group Name`, `Group Category`) |>
  summarise(`Sample Size` = sum(`Sample Size`),
            n_hesitant = sum(n_hesitant)) |>
  mutate(`Estimate (%)` = n_hesitant / `Sample Size` * 100,
         `Time Period` = "2021 analysis period",
...
state_group_analysis_pivot <- state_group_analysis |>
  ...
  select(state_clean, `Estimate (%)`, concat) |>
  pivot_wider(id_cols = state_clean, names_from = "concat",
...
top_10_hesistant_states <- covid_2021_analysis_allstates |>
  ...
  filter(!state_clean %in% c("Guam")) |>
  slice(1:10)
  
# Identify top least hesitant states
# Filtering out Guam and other small territories. Keep PR?
...
top_10_parallel_analaysis <- state_group_analysis_pivot |>
  filter(state_clean %in% top_10_hesistant_states$state_clean)


# Display parallel coordinates plot for most hesitant states
ggparcoord(top_10_parallel_analaysis, columns = c(2:8),
           scale = "globalminmax",
           alpha = 0.4) +
...
top_10_parallel_analaysis |>
  select(-c(`Age.Race - 18 – 49, Other`, `Pregnancy - Trying to get pregnant`)) |>
...
bottom_10_parallel_analaysis <- state_group_analysis_pivot |>
  filter(state_clean %in% bottom_10_hesistant_states$state_clean)
